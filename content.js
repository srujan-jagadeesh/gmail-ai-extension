  ...
  sendButtons.forEach((sendBtn) => {
    ...
    menu.innerHTML = `
      ...
      <div class="gemini-menu-body">
        <div style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 10px;">
          ...
          <button class="gemini-chip gemini-donate-btn" style="background: #dcfce7; color: #15803d; border-color: #86efac; font-weight: 600;">❤️ Donate</button>
        </div>
    ...
    if (!document.getElementById("gemini-extension-style")) {
      ...
      style.textContent = `
        ...
        .gemini-donate-btn:hover {
          background: #bbf7d0 !important;
        }
        #gemini-qr-code img, #gemini-qr-code canvas {
          margin: 0 auto;
          border-radius: 8px;
        }
      `;
    ...
    // "❤️ Donate" Greenish Payment Modal Handler
    const donateBtn = menu.querySelector(".gemini-donate-btn");
    donateBtn.addEventListener("click", (e) => {
      e.stopPropagation();

      const upiUrl = "upi://pay?pa=srujansrujan128-1@oksbi&pn=Srujan&cu=INR&tn=Donate";
      ...
      if (!overlay) {
        ...
        overlay.innerHTML = `
          <div style="background: #1e293b; color: #f8fafc; padding: 20px; border-radius: 16px; text-align: center; width: 300px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.5); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; position: relative; border: 1px solid rgba(255,255,255,0.1);">
            ...
            <img src="${profileImgUrl}" alt="Profile" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; border: 2.5px solid #22c55e; margin: 0 auto 10px auto; display: block;" />
            
            <h3 style="margin: 0 0 4px 0; font-size: 16px; font-weight: 700; color: #ffffff;">Support the Project ❤️</h3>
            <p style="margin: 0 0 16px 0; font-size: 12px; color: #94a3b8; line-height: 1.3;">Every contribution helps keep srujan mail ai tool free and growing!</p>

            <div style="border-top: 1px dashed rgba(255, 255, 255, 0.15); margin-bottom: 16px;"></div>

            <!-- Local QR Container -->
            <div id="gemini-qr-code" style="background: #ffffff; padding: 10px; border-radius: 12px; width: 160px; height: 160px; margin: 0 auto; display: flex; align-items: center; justify-content: center;"></div>
            
            <div style="margin-top: 10px; font-size: 11px; color: #94a3b8; line-height: 1.4;">
              Scan via GPay / PhonePe / Paytm<br>
              <b style="color: #4ade80; font-size: 11px;">srujansrujan128-1@oksbi</b>
            </div>
          </div>
        `;
        document.body.appendChild(overlay);

        // Render QR Code via local library
        if (window.QRCode) {
          new QRCode(document.getElementById("gemini-qr-code"), {
            text: upiUrl,
            width: 150,
            height: 150,
            colorDark: "#000000",
...
- Do NOT include Markdown formatting (no ###, **, >) or Subject headers.`;
    ...
    menu.querySelectorAll(".gemini-chip:not(.gemini-donate-btn)").forEach((chip) => {
